...
import { searchVinted, findCheaperAlternatives as findCheaperVinted, } from "./vinted-scraper.js";
import { searchKleinanzeigen, } from "./kleinanzeigen-scraper.js";
...
function buildSearchText(brand, categoryKey) {
    const cat = CATEGORIES[categoryKey];
    if (!cat || !cat.keyword)
        return brand;
    return `${brand} ${cat.keyword}`;
}
async function findChannelInSection(client, channelName, sectionName) {
    for (const [, guild] of client.guilds.cache) {
        const category = guild.channels.cache.find((c) => c.name.toLowerCase() === sectionName.toLowerCase() && c.type === 4);
        if (!category)
            continue;
        const ch = guild.channels.cache.find((c) => c.name === channelName && c instanceof TextChannel && c.parentId === category.id);
        if (ch)
            return ch;
    }
    return null;
...
async function notifyBlocked(client) {
    if (Date.now() - lastBlockedNotify < 60 * 60 * 1000)
        return;
    lastBlockedNotify = Date.now();
    const embed = new EmbedBuilder()
        .setColor(0xff4444)
        .setTitle("⚠️ Vinted-Token abgelaufen")
        .setDescription("Der Bot kann nicht mehr auf Vinted zugreifen.\n\n" +
...
const commands = [
    new SlashCommandBuilder()
        ...
        .addSubcommand((sub) => sub.setName("marken").setDescription("Marken einstellen (kommagetrennt)")
        .addStringOption((o) => o.setName("liste").setDescription("z.B. Nike,Adidas,Lacoste").setRequired(true)))
        .addSubcommand((sub) => sub.setName("maxpreis").setDescription("Maximalen Preis in EUR einstellen")
        .addIntegerOption((o) => o.setName("preis").setDescription("z.B. 50 für max. 50 EUR (0 = kein Limit)").setRequired(true)))
        .addSubcommand((sub) => sub.setName("kategorie").setDescription("Nur eine bestimmte Kategorie suchen")
        .addStringOption((o) => o.setName("typ").setDescription("Kategorie auswählen").setRequired(true).addChoices(...CATEGORY_CHOICES)))
        .addSubcommand((sub) => sub.setName("geschlecht").setDescription("Nach Herren / Damen / Beide filtern")
        .addStringOption((o) => o.setName("typ").setDescription("Geschlecht auswählen").setRequired(true)
        ...
        .addSubcommand((sub) => sub.setName("token").setDescription("Vinted access_token_web Cookie manuell setzen")
...
export async function startBot() {
    ...
    if (!token) {
        logger.error("DISCORD_BOT_TOKEN ist nicht gesetzt.");
        return;
    }
    ...
    client.once(Events.ClientReady, async (c) => {
        ...
        try {
            ...
            if (botOwnerId)
                logger.info(`Bot-Owner erkannt: ${botOwnerId}`);
        }
        catch (err) {
        ...
        if (interaction.isButton()) {
            ...
            if (customId.startsWith("pricecheck_")) {
                ...
                const mainEmbed = new EmbedBuilder()
                    ...
                    .setTimestamp();
                if (item.imageUrl)
                    mainEmbed.setThumbnail(item.imageUrl);
                ...
                if (alternatives.length === 0) {
                    ...
                    for (const [i, alt] of alternatives.slice(0, 3).entries()) {
                        ...
                        allEmbeds.push(new EmbedBuilder()
                            ...
                            .setURL(alt.url)
                            .addFields({ name: "💰 Preis", value: `**${alt.price.toFixed(2)} ${alt.currency}**`, inline: true }, { name: "💸 Ersparnis", value: `**-${savings.toFixed(2)} EUR**`, inline: true }, { name: "📐 Größe", value: alt.size || "—", inline: true }, { name: "✨ Zustand", value: alt.condition || "—", inline: true }, { name: "👤 Verkäufer", value: alt.seller || "—", inline: true })
                            .setThumbnail(alt.imageUrl));
                        allRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel(`#${i + 1} Ansehen`).setStyle(ButtonStyle.Link).setURL(alt.url)));
                    }
                }
                try {
                    const dm = await user.createDM();
            ...
            if (customId.startsWith("save_")) {
                ...
                const savedEmbed = new EmbedBuilder()
                    ...
                    .setURL(item.url)
                    .addFields({ name: "💰 Preis", value: `**${item.price.toFixed(2)} ${item.currency}**`, inline: true }, { name: "📐 Größe", value: item.size || "—", inline: true }, { name: "✨ Zustand", value: item.condition || "—", inline: true }, { name: "👤 Verkäufer", value: item.seller || "—", inline: true })
                    .setFooter({ text: "Deine gemerkten Deals • Snipebot" })
                    .setTimestamp();
                if (item.imageUrl)
                    savedEmbed.setImage(item.imageUrl);
                const platformName = item.platform === "vinted" ? "Vinted" : "Kleinanzeigen";
                const linkRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel(`🛒 Auf ${platformName} ansehen`).setStyle(ButtonStyle.Link).setURL(item.url));
                try {
                    const dm = await user.createDM();
                    await dm.send({
        ...
        try {
            if (cmd.commandName === "deals") {
                ...
                if (sub === "start") {
                    ...
                    if (typ !== "alle" && !CATEGORIES[typ]) {
                        await cmd.reply("❌ Unbekannte Kategorie.");
                        return;
                    }
